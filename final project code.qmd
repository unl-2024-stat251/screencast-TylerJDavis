...
roster_new <- rosters[!duplicated(rosters$player_id), ]
rosters_t2 <- roster_new %>%
  drop_na()%>%
  filter(birth_country == "CAN")%>%
  group_by(year) %>%
...
ggplot(birth_year_graph,aes(year,avg_w_p, size = year_count)) + geom_count() + ggtitle("Average Weight per Birth Year in the NHL from 1995 - 2005 for Canadian Born Players") + ylab("Average Weight in Pounds") + xlab("Year")
